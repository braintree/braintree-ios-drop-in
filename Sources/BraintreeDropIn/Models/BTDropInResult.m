...
+ (void)fetchDropInResultForAuthorization:(NSString *)authorization handler:(BTDropInResultFetchHandler)handler {
    BTDropInPaymentMethodType lastSelectedPaymentMethodType = [[NSUserDefaults standardUserDefaults] integerForKey:@"BT_dropInLastSelectedPaymentMethodType"];
    ...
    [apiClient fetchPaymentMethodNonces:NO completion:^(NSArray<BTPaymentMethodNonce *> *paymentMethodNonces, NSError *error) {
        if (error != nil) {
            ...
            if (lastSelectedPaymentMethodType == BTDropInPaymentMethodTypeApplePay) {
                result.paymentMethodType = lastSelectedPaymentMethodType;
